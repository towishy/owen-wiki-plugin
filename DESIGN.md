...
title: "Owen Wiki Plugin Design"
status: adopted
product: "owen-wiki-plugin"
surface: "Obsidian setup wizard, safety review and health report"
kind: plugin
updated: 2026-08-12
generated_by: "wiki/scripts/generate-project-design-docs.py"
foundation_guide_source: "D:\\Github\\wiki\\UI-FOUNDATION-DESIGN-GUIDE.md"
foundation_guide_sha256: "5631b2f20a922c13be6ed2fffd36f4506bf3e5bf515822db5287ea234c48261c"
---

<!-- markdownlint-disable MD013 MD024 MD025 MD033 -->

# Owen Wiki Plugin Design

## Decision Summary

| 항목 | 결정 |
| --- | --- |
| 사용자 | 기존 Obsidian vault에 Owen Wiki 구조를 설치하는 사용자 |
| Output job | Preset과 overwrite·backup 범위를 검토해 vault를 안전하게 scaffold한다 |
| Primary object | 설치 preset, 생성·교체 file plan과 health result |
| Primary action | Review된 plan을 vault에 적용한다 |
| Composition | Native Obsidian modal wizard + review table + success actions |
| 핵심 판단 | 파일 쓰기 전에 dry run·backup·overwrite 범위를 명시하고 native Obsidian semantics를 유지한다 |

## Context

Owen Wiki Plugin은 template kit를 active vault에 bootstrap한다. Minimal, standard, full, custom preset과 dry run, backup, health check가 핵심 state다.
...
- Native Obsidian modal과 form control
- Preset·safety·review·install·result 단계
- File plan은 opaque table
- Destructive overwrite는 backup과 explicit review 필요

### Excluded Or App-Owned

- 한 화면에 모든 option 노출
- 사용자 확인 없는 overwrite
- 설치 성공과 health pass를 같은 상태로 간주

## Design Decisions

### D-01. 설치는 reviewable plan이다

- **결정**: Preset 선택 후 create, overwrite, skip file count와 path를 review한다
- **이유**: Vault mutation의 범위를 실행 전에 이해해야 한다
- **검증**: Dry run과 actual result manifest를 비교한다

### D-02. Backup은 overwrite와 결박한다

- **결정**: Overwrite 대상과 backup location을 함께 표시하고 backup 실패 시 쓰기를 중단한다
- **이유**: 기존 지식을 silent loss하지 않기 위해서다
- **검증**: Backup success/failure와 restore smoke를 실행한다

### D-03. 완료 후 next action을 제공한다

- **결정**: Open AGENTS, overview, health check command를 success state에 둔다
- **이유**: Scaffold 생성만으로 사용 준비가 끝나지 않는다
- **검증**: Shortcut target 존재와 focus restore를 확인한다

## State Contract

| 상태 | 표현 | 다음 행동 |
| --- | --- | --- |
| Loading | 현재 수행 중인 작업과 기다리는 대상을 표시한다 | 취소·대기 또는 준비된 기능 사용 |
| Empty | 빈 이유와 생성·연결 경로를 설명한다 | 첫 항목 생성 또는 source 연결 |
| Error | 실패 범위와 원인을 전체 실패처럼 과장하지 않는다 | 실패한 작업만 재시도 |
| Partial | 사용 가능한 결과와 누락된 범위를 함께 표시한다 | 제한된 작업을 계속하거나 보강 |
| Success | 결과 위치와 다음 행동을 가까이 표시한다 | 검토·저장·내보내기 |

## Responsive Or Delivery Contract

| 범위 | 유지·변경 | 금지 |
| --- | --- | --- |
| Desktop | 주 navigation·work plane·보조 정보의 책임을 유지 | 장식 card 중첩을 만들지 않는다 |
| Tablet | column 수를 줄이고 inspector를 아래로 이동 | 본문 font를 축소하지 않는다 |
| Mobile | 한 열 reading order와 44px target을 유지 | horizontal overflow와 hover-only action을 허용하지 않는다 |

## Accessibility And Safety Contract

- 색상만으로 상태·선택·오류를 전달하지 않는다.
- keyboard·touch·pointer에서 같은 핵심 작업과 recovery path를 제공한다.
- focus-only outline·ring·border·shadow를 새로 만들지 않고 Foundation의 상태 계약을 따른다.
- reduced motion, forced colors, text zoom 또는 host accessibility 설정을 우회하지 않는다.
- 비밀값·PII·고객 데이터는 화면 sample, screenshot, log, fixture에 넣지 않는다.

## Validation

- npm test and build
- Temporary Obsidian vault smoke
- Dry run·backup·overwrite·health result comparison

## Change Rules

- Primary action, information order, responsive responsibility 또는 안전 경계가 바뀌면 이 문서를 갱신한다.
- Foundation public API·reference composition·semantic token 선택이 바뀌면 근거와 검증을 함께 갱신한다.
- 일회성 장식 수정은 기록하지 않되 둘 이상의 surface에서 반복되면 공통 결정 후보로 승격한다.
- 아래 Foundation guide snapshot은 generator로만 갱신하고 수동 편집하지 않는다.

<!-- ui-foundation-design-guide:snapshot:start sha256=5631b2f20a922c13be6ed2fffd36f4506bf3e5bf515822db5287ea234c48261c source=D:\Github\wiki\UI-FOUNDATION-DESIGN-GUIDE.md -->

<details>
<summary>UI-FOUNDATION-DESIGN-GUIDE.md 전체 원문</summary>
...
<!-- ui-foundation-design-guide:snapshot:end -->
